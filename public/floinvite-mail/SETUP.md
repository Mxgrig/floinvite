...
```
Database Name: REDACTED_USER_mail
Database User: REDACTED_USER_mail
...
1. **Via phpMyAdmin (Hostinger Control Panel)**:
   ...
   - Select the database: `REDACTED_USER_mail`
   - Click "Import" tab
...
2. **Via Command Line** (if SSH access available):
   ```bash
   mysql -u REDACTED_USER_mail -p REDACTED_USER_mail < schema.sql
   # When prompted, enter password: your-db-password
   ```

3. **Via MySQL Client**:
   ```bash
   mysql -h localhost -u REDACTED_USER_mail -p 'your-db-password' REDACTED_USER_mail < schema.sql
   ```
...
```bash
mysql -u REDACTED_USER_mail -p 'your-db-password' REDACTED_USER_mail -e "SHOW TABLES;"
```
...
- Check user privileges (should have all on REDACTED_USER_mail.*)
```
...
- `.htpasswd` location in production: `/home/REDACTED_USER/domains/floinvite.com/public_html/floinvite-mail/.htpasswd`
