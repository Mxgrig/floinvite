...
1. **Create MySQL Database** (if not exists)
   ...
   # Create: REDACTED_USER_floinvite
   # User: REDACTED_USER_floinvite
   # Password: [your password]
   ```

2. **Upload PHP Files**
   ```bash
   scp -P 65002 public/api/db-setup.php REDACTED_USER@REDACTED_HOST:~/public_html/api/
   scp -P 65002 public/api/subscription-status.php REDACTED_USER@REDACTED_HOST:~/public_html/api/
   scp -P 65002 public/api/check-checkin-allowed.php REDACTED_USER@REDACTED_HOST:~/public_html/api/
   scp -P 65002 public/api/webhooks/stripe-v2.php REDACTED_USER@REDACTED_HOST:~/public_html/api/webhooks/
   ```
...
3. **Initialize Database**
   ...
   # Option 2: Via SSH
   ssh -p 65002 REDACTED_USER@REDACTED_HOST "php ~/public_html/api/db-setup.php"
   ```
...
scp -r -P 65002 dist/* REDACTED_USER@REDACTED_HOST:~/public_html/
```
...
DB_HOST=localhost
DB_NAME=REDACTED_USER_floinvite
DB_USER=REDACTED_USER_floinvite
...
1. **Rollback Frontend**:
   ...
   npm run build
   scp -r dist/* REDACTED_USER@REDACTED_HOST:~/public_html/
   ```
