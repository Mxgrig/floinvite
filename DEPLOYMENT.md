...
# Should show: REDACTED_HOST (Hostinger IP)
...
3. **Update A Record for `@` (main domain)**:
   - Type: A
   - Name: @
   - IPv4 Address: `REDACTED_HOST` (Hostinger server IP)
   - Proxy: **DNS only** (⚠️ NOT "Proxied"!)
   - TTL: Auto
4. **Update A Record for `www`**:
   - Type: A
   - Name: www
   - IPv4 Address: `REDACTED_HOST`
   - Proxy: **DNS only**
   - TTL: Auto
...
3. **Update A record to point to Hostinger IP**: `REDACTED_HOST`
...
- ✅ DNS records point to REDACTED_HOST
...
scp -P 65002 -r /home/grig/Projects/floinvite/dist/* \
    REDACTED_USER@REDACTED_HOST:~/public_html/
...
1. Open browser to: `https://REDACTED_HOST` or your domain
2. Check for:
...
1. **Update DNS Records:**
   - A Record: Point to `REDACTED_HOST`
...
2. Connection: `REDACTED_HOST` port `65002`
